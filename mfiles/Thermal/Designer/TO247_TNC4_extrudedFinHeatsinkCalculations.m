...
areaContactCopper = 140*10^-6;
areaContact = areaContactCopper + 10*10^-6; % correction of effective area since heat spreading used only account for spreading to the outside, not in hole
widthDissipation = 20*10^-3;
lengthDissipation = 20*10^-3;
...
rThTim = timLayer.thermalLayerResistance(areaContactCopper);
insTim = rThTim*areaContact;

rThTot = rThTim + rThSink + 0.35
% hCase = 1/(rThTot*areaContact);
% spreadLayerChip = ThermalLayer(0.4*10^-3, 385);
% areaChip = 40*10^-6;
